...
audio1 = AudioSegment.from_file("audio-files/timer.wav")
audio1_justname = "audio-files/timer.wav"
# Initialize audio2 files
audio2 = AudioSegment.from_file("audio-files/Windows XP Startup.wav")
audio2_justname = "audio-files/Windows XP Startup.wav"
...
def which_audio(sel: int):
    ...
    try:
        if which_audio_sel == 1:
            return audio1, audio1, "audio-files/timer.wav"
        if which_audio_sel == 2:
            return audio2, audio2, "audio-files/Windows XP Startup.wav"
        else:
            raise ValueError
    except ValueError:
...
def duration(audio_for_use: AudioSegment, text_name: str):
    print(f"The audio is {audio_for_use[:11000].duration_seconds:.2f} seconds long.")
    print("Displaying Plot.")
    if text_name == 'audio-files/timer.wav':
        plot_indexed_audio(text_name, 0, 11)
    elif text_name == 'audio-files/Windows XP Startup.wav':
        plot_full_audio(text_name)
...
def update_frame_rate():
    ...
    updated_audio.export("audio-files/updated_audio_2.wav", format="wav")
    time.sleep(2)
    print("Playing updated audio.")
    play(updated_audio[:11000])
    print("Displaying Plot.")
    new_file = "audio-files/updated_audio_2.wav"
